...
class M_Penerimaan extends CI_Model
{
	...
	public function GetKodePo()
	{
		...
		if ($is_dewa > 0) {
			...
			and po.depo_id = '$depo_id' and pr.tipe_pengadaan_id ='790A3E3A-C78F-4845-BDC9-043E3BECFECB'
			")->result_array();
		} else {
			if ($client_wms_id != '') {

				return $this->db->query("select * from purchase_order po
				left join purchase_request pr on po.purchase_request_id = pr.purchase_request_id where po.purchase_order_status !='Completed' and po.purchase_order_status !='In Progress Receiving' and pr.tipe_pengadaan_id ='790A3E3A-C78F-4845-BDC9-043E3BECFECB'
	...
	public function GetPurchaseRequestDetailByIdEdit($id, $penerimaan_id)
	{
		...
		penerimaan.penerimaan_sku_barang_id,
		penerimaan_detail.sku_barang_id,
		penerimaan_detail.sku_barang_qty,
		penerimaan_detail.sku_barang_qty_terima as po_akan_diterima,
		penerimaan_detail.sku_barang_qty,
		...
		left join penerimaan_sku_barang penerimaan
		on penerimaan.purchase_order_id = po.purchase_order_id
		left join penerimaan_sku_barang_detail penerimaan_detail
		on penerimaan_detail.penerimaan_sku_barang_id = penerimaan.penerimaan_sku_barang_id
		and penerimaan_detail.sku_barang_id = po_detail.sku_barang_id
		LEFT JOIN sku_barang
			ON penerimaan_detail.sku_barang_id = sku_barang.sku_barang_id
			left join purchase_request pr on pr.purchase_request_id = po.purchase_request_id
		left join supplier s on s.supplier_id = po.supplier_id
		where po.purchase_order_id = '$id' AND penerimaan.penerimaan_sku_barang_id= '$penerimaan_id'
		ORDER BY sku_barang.sku_barang_kode ASC
");
		// 	$query = $this->db->query("
		// 	SELECT 
	...
	public function delete_penerimaan_sku_barang_detail($penerimaan_id, $detail)
	{
		...
		$this->db->where("penerimaan_sku_barang_id", $penerimaan_id);
		$this->db->where("sku_barang_id", $sku_barang_id);

		$this->db->delete("penerimaan_sku_barang_detail");
	...
	public function update_penerimaan_sku_barang($penerimaan_id, $purchase_order_id, $depo_id, $gudang_id, $client_wms_id, $po_status)
	{
		$this->db->set('gudang_id', $gudang_id);
		$this->db->set('penerimaan_sku_barang_status', $po_status);
		$this->db->where("penerimaan_sku_barang_id", $penerimaan_id);
		$this->db->where("purchase_order_id", $purchase_order_id);
		$this->db->update('penerimaan_sku_barang');
		$affectedrows = $this->db->affected_rows();
		if ($affectedrows > 0) {
			$querydelete = 1;
		} else {
