...
class M_AllotmentStockOrderManager extends CI_Model
{
	function __construct()
	{
		// Call the Model constructor
		parent::__construct();
		// $this->karyawan_manager_id_global = "00350571-BDE8-46B5-AFB0-01FFDD33484C";
		$this->karyawan_manager_id_global = $this->session->userdata('karyawan_id');
	}
	...
	public function Get_list_karyawan($perusahaan, $karyawan_manager_id, $filter_karyawan)
	{
		$filter_karyawan_str = "";
		...
		$query = $this->db->query("select
									...
									and karyawan_supervisor_id = '$karyawan_manager_id'
	...
	public function Get_list_allotment_stock_order_detail_summary($temp_id, $tahun, $filter_sku)
	{
		...
		$query = $this->db->query("SELECT bod.sku_id,
										...
										brand.principle_brand_nama AS brand,
										SUM(ISNULL(bod.sku_qty, 0)) AS sku_qty,
										ISNULL(mng.sku_subtotal_qty_mng, 0) AS sku_qty_mng,
										ISNULL(mng.sku_subtotal_qty_sup, 0) AS sku_qty_sup,
										CASE WHEN ISNULL(mng.sku_subtotal_qty_mng, 0) <> ISNULL(mng.sku_subtotal_qty_sup, 0) THEN '1' ELSE '0' END as is_berubah
									FROM back_order bo
									INNER JOIN back_order_detail bod ON bod.back_order_id = bo.back_order_id
									LEFT JOIN sku ON sku.sku_id = bod.sku_id
									LEFT JOIN principle ON principle.principle_id = sku.principle_id
									...
									LEFT JOIN
									(SELECT dtl.sku_id,
											sum(isnull(a.sku_subtotal_qty, 0))  AS sku_subtotal_qty_mng,
											sum(isnull(b.sku_subtotal_qty, 0)) AS sku_subtotal_qty_sup
									FROM allotment_stock_order_detail2_manager a
									LEFT JOIN (select allotment_stock_order_detail2_manager_id, sum(isnull(sku_subtotal_qty, 0)) as sku_subtotal_qty from allotment_stock_order_detail2_supervisor group by allotment_stock_order_detail2_manager_id) b 
									...
									AND convert(nvarchar(36), a.karyawan_manager_id) = '$this->karyawan_manager_id_global'
									...
									GROUP BY bod.sku_id,
											...
											brand.principle_brand_nama,
											ISNULL(sim.qty, 0),
											ISNULL(mng.sku_subtotal_qty_mng, 0),
											ISNULL(mng.sku_subtotal_qty_sup, 0)
	...
	public function Get_list_karyawan_supervisor_simulasi_mps($temp_id, $tahun, $bulan, $sku_id, $karyawan_manager_id, $filter_karyawan, $arr_list_karyawan_supervisor_simulasi)
	{
		...
		if (isset($arr_list_karyawan_supervisor_simulasi) && count($arr_list_karyawan_supervisor_simulasi) > 0) {
			...
			$query = $this->db->query("select
										...
										and a.bulan = '$bulan'
										and a.sku_id = '$sku_id'
										and convert(nvarchar(36), a.karyawan_manager_id) = '$karyawan_manager_id'
										" . $filter_karyawan_str . "
										group by a.karyawan_supervisor_id,
												a.karyawan_manager_id,
												karyawan.karyawan_nama,
												divisi.karyawan_divisi_nama,
												level.karyawan_level_nama,
	...
	public function Get_proses_simulasi_mps($tahun, $sku_id, $temp_id)
	{
		...
		$query = $this->db->query("exec proses_simulasi_mps_manager $tahun, '$sku_id', '$temp_id', '$this->karyawan_manager_id_global'");
