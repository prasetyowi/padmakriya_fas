...
class M_AllotmentStockOrderSupervisor extends CI_Model
{
	function __construct()
	{
		// Call the Model constructor
		parent::__construct();
		// $this->karyawan_supervisor_id_global = "44B26394-11B9-4926-A0F0-BB351939DCD7";
		$this->karyawan_supervisor_id_global = $this->session->userdata('karyawan_id');
	}
	...
	public function Get_list_karyawan($perusahaan, $karyawan_supervisor_id, $filter_karyawan)
	{
		$filter_karyawan_str = "";
		...
		$query = $this->db->query("select
									...
									and karyawan_supervisor_id = '$karyawan_supervisor_id'
									--karyawan.client_wms_id = '$perusahaan'
									" . $filter_karyawan_str . "
	...
	public function Get_list_sku_back_order($tahun, $perusahaan, $principle, $filter_sku)
	{
		...
		$query = $this->db->query("SELECT bod.sku_id,
									...
									WHERE bo.back_order_id is not null
	...
	public function Get_list_allotment_stock_order_detail_summary($temp_id, $tahun, $filter_sku)
	{
		...
		$query = $this->db->query("SELECT bod.sku_id,
										...
										brand.principle_brand_nama AS brand,
										SUM(ISNULL(bod.sku_qty, 0)) AS sku_qty,
										ISNULL(mng.sku_subtotal_qty_spv, 0) AS sku_subtotal_qty_spv,
										ISNULL(mng.sku_subtotal_qty_sls, 0) AS sku_subtotal_qty_sls,
										CASE WHEN ISNULL(mng.sku_subtotal_qty_spv, 0) <> ISNULL(mng.sku_subtotal_qty_sls, 0) THEN '1' ELSE '0' END as is_berubah
									FROM back_order bo
									INNER JOIN back_order_detail bod ON bod.back_order_id = bo.back_order_id
									LEFT JOIN sku ON sku.sku_id = bod.sku_id
									LEFT JOIN principle ON principle.principle_id = sku.principle_id
									...
									GROUP BY sku_id,
											tahun_kirim) sim ON sim.sku_id = bod.sku_id
									LEFT JOIN
									(SELECT dtl.sku_id,
											sum(isnull(a.sku_subtotal_qty, 0))  AS sku_subtotal_qty_spv,
											sum(isnull(b.sku_subtotal_qty, 0)) AS sku_subtotal_qty_sls
									FROM allotment_stock_order_detail2_supervisor a
									LEFT JOIN (select allotment_stock_order_detail2_supervisor_id, sum(isnull(sku_subtotal_qty, 0)) as sku_subtotal_qty from allotment_stock_order_detail2_sales group by allotment_stock_order_detail2_supervisor_id) b 
									ON b.allotment_stock_order_detail2_supervisor_id = a.allotment_stock_order_detail2_supervisor_id
									LEFT JOIN allotment_stock_order_detail2 dtl2 ON dtl2.allotment_stock_order_detail2_id = a.allotment_stock_order_detail2_id
									LEFT JOIN allotment_stock_order_detail dtl ON dtl.allotment_stock_order_detail_id = dtl2.allotment_stock_order_detail_id
									WHERE a.allotment_stock_order_id = '$temp_id'
									AND convert(nvarchar(36), a.karyawan_supervisor_id) = '$this->karyawan_supervisor_id_global'
									GROUP BY dtl.sku_id) mng ON mng.sku_id = bod.sku_id
									WHERE bo.back_order_status = 'Approved'
									AND bo.depo_id = '" . $this->session->userdata('depo_id') . "'
									" . $filter_sku_str . "
									GROUP BY bod.sku_id,
											sku.sku_kode,
											sku.sku_nama_produk,
											...
											brand.principle_brand_nama,
											ISNULL(sim.qty, 0),
											ISNULL(mng.sku_subtotal_qty_spv, 0),
											ISNULL(mng.sku_subtotal_qty_sls, 0)
	...
	public function Get_list_karyawan_supervisor_simulasi_mps($temp_id, $tahun, $bulan, $sku_id, $filter_karyawan, $arr_list_karyawan_supervisor_simulasi)
	{

		$filter_karyawan_str = "";
		if (isset($filter_karyawan) && count($filter_karyawan) > 0) {
			foreach ($filter_karyawan as $value) {
				$filter_karyawan_str = " AND convert(nvarchar(36), a.karyawan_supervisor_id) IN (" . implode(",", $filter_karyawan) . ")";
			}
		} else {
			$filter_karyawan_str = " AND convert(nvarchar(36), a.karyawan_supervisor_id) IN ('')";
		}

		$union = " UNION ALL ";
		$table_sementara = "";
		if (isset($arr_list_karyawan_supervisor_simulasi) && count($arr_list_karyawan_supervisor_simulasi) > 0) {
			foreach ($arr_list_karyawan_supervisor_simulasi as $key => $value) {

				$table_sementara .= "SELECT '" . $value['karyawan_supervisor_id'] . "' AS karyawan_supervisor_id, '" . $value['sku_id'] . "' AS sku_id, '" . $value['tahun'] . "' AS tahun, '" . $value['bulan'] . "' AS bulan, " . $value['sku_qty'] . " AS sku_qty ";

				if ($key < count($arr_list_karyawan_supervisor_simulasi) - 1) {
					$table_sementara .= $union;
				}
			}
			$query = $this->db->query("select
										karyawan.karyawan_id as karyawan_supervisor_id,
										karyawan.karyawan_nama,
										divisi.karyawan_divisi_nama as divisi,
										level.karyawan_level_nama as level,
										a.sku_id,
										...
										left join karyawan
										on karyawan.karyawan_id = a.karyawan_supervisor_id
										left join karyawan_divisi divisi
										on divisi.karyawan_divisi_id = karyawan.karyawan_divisi_id
										left join karyawan_level level
										on level.karyawan_level_id = karyawan.karyawan_level_id
	...
	public function Get_list_karyawan_sales_simulasi_mps($temp_id, $tahun, $bulan, $sku_id, $karyawan_supervisor_id, $filter_karyawan, $arr_list_karyawan_supervisor_simulasi)
	{

		$filter_karyawan_str = "";
		if (isset($filter_karyawan) && count($filter_karyawan) > 0) {
			foreach ($filter_karyawan as $value) {
				$filter_karyawan_str = " AND convert(nvarchar(36), a.karyawan_sales_id) IN (" . implode(",", $filter_karyawan) . ")";
			}
		} else {
			$filter_karyawan_str = " AND convert(nvarchar(36), a.karyawan_sales_id) IN ('')";
		}
		...
		if (isset($arr_list_karyawan_supervisor_simulasi) && count($arr_list_karyawan_supervisor_simulasi) > 0) {
			...
			$query = $this->db->query("select
										...
										from (" . $table_sementara . ") a
										left join karyawan
										on karyawan.karyawan_id = a.karyawan_sales_id
										left join karyawan_divisi divisi
										on divisi.karyawan_divisi_id = karyawan.karyawan_divisi_id
										left join karyawan_level level
										on level.karyawan_level_id = karyawan.karyawan_level_id
										...
										and a.bulan = '$bulan'
										and a.sku_id = '$sku_id'
										and a.karyawan_supervisor_id = '$karyawan_supervisor_id'
										" . $filter_karyawan_str . "
										group by a.karyawan_sales_id,
												a.karyawan_supervisor_id,
												karyawan.karyawan_nama,
												divisi.karyawan_divisi_nama,
												level.karyawan_level_nama,
	...
	public function Get_allotment_stock_order_detail2_by_karyawan_supervisor($allotment_stock_order_id, $allotment_stock_order_tahun, $back_order_simulasi)
	{
		$union = " UNION ALL ";
		$table_sementara = "";
		if (isset($back_order_simulasi) && count($back_order_simulasi) > 0) {
	...
	public function Get_allotment_stock_order_by_filter($tahun, $perusahaan, $status)
	{
		...
		$query = $this->db->query("SELECT a.allotment_stock_order_id,
										a.client_wms_id,
										...
										COUNT(DISTINCT b.allotment_stock_order_id) AS is_berubah
									FROM allotment_stock_order a
									LEFT JOIN
									(SELECT a.allotment_stock_order_id,
											a.allotment_stock_order_detail2_manager_id,
											a.sku_subtotal_qty AS sku_subtotal_qty_mng,
											sum(isnull(b.sku_subtotal_qty, 0)) AS sku_subtotal_qty_sup
									FROM allotment_stock_order_detail2_manager a
									LEFT JOIN allotment_stock_order_detail2_supervisor b ON b.allotment_stock_order_detail2_manager_id = a.allotment_stock_order_detail2_manager_id
									GROUP BY a.allotment_stock_order_id,
												a.allotment_stock_order_detail2_manager_id,
												a.sku_subtotal_qty
									HAVING a.sku_subtotal_qty <> sum(isnull(b.sku_subtotal_qty, 0))) b ON b.allotment_stock_order_id = a.allotment_stock_order_id
									LEFT JOIN client_wms perusahaan ON perusahaan.client_wms_id = a.client_wms_id
									...
									GROUP BY a.allotment_stock_order_id,
											...
											a.allotment_stock_order_kode,
											a.allotment_stock_order_tahun,
											ISNULL(a.allotment_stock_order_status, ''),
											a.allotment_stock_order_tgl_create,
											a.allotment_stock_order_who_create,
											a.allotment_stock_order_keterangan,
											a.allotment_stock_order_tgl_update,
											a.allotment_stock_order_who_update
									ORDER BY a.allotment_stock_order_tahun DESC,
											a.allotment_stock_order_kode ASC");
	...
	public function Get_proses_simulasi_mps($tahun, $sku_id, $temp_id)
	{
		...
		$query = $this->db->query("exec proses_simulasi_mps_supervisor $tahun, '$sku_id', '$temp_id', '$this->karyawan_supervisor_id_global'");
	...
	public function delete_allotment_stock_order_detail2_supervisor($allotment_stock_order_id)
	{

		$this->db->where('allotment_stock_order_id', $allotment_stock_order_id);
		$queryinsert = $this->db->delete("allotment_stock_order_detail2_supervisor");
