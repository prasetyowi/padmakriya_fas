...
class M_PurchaseOrder extends CI_Model
{
	...
	public function search_purchase_request_by_filter($tgl1, $tgl2, $perusahaan, $divisi)
	{
		if ($perusahaan == "") {
			$perusahaan = "";
		} else {
		...
		if ($divisi == "") {
			...
			$divisi = "AND purchase_order.karyawan_divisi_id = '" . $divisi . "' ";
		}
		...
		$query = $this->db->query("SELECT distinct
                                    purchase_request.purchase_request_kode,
										...
										purchase_order.karyawan_divisi_id,
										karyawan_divisi.karyawan_divisi_nama,
										purchase_order.purchase_order_id,
										purchase_order.purchase_order_kode,
										tipe_transaksi.tipe_transaksi_nama,
										FORMAT(purchase_order.purchase_order_tgl_create,'dd-MM-yyyy') AS purchase_order_tgl,
										purchase_order.purchase_order_status,
										purchase_order.purchase_order_who_create,
										FORMAT(purchase_order.purchase_order_tgl_create,'dd-MM-yyyy') AS purchase_order_tgl_create,
									...
									ON purchase_order.purchase_order_id = purchase_order_detail.purchase_order_id
                                    left join supplier 
									...
									LEFT JOIN approval
									ON purchase_order.purchase_order_id = approval.approval_reff_dokumen_id
									LEFT JOIN karyawan approval_by
									ON approval_by.karyawan_id = approval.approval_karyawan_id
									LEFT JOIN karyawan_divisi
									ON karyawan_divisi.karyawan_divisi_id = purchase_order.karyawan_divisi_id
									...
									WHERE FORMAT(purchase_order.purchase_order_tgl_create,'yyyy-MM-dd') BETWEEN '$tgl1' AND '$tgl2'
									AND purchase_order.depo_id = '" . $this->session->userdata('depo_id') . "'
									--AND purchase_order.client_wms_id IN (SELECT client_wms_id FROM karyawan_principle WHERE karyawan_id = '" . $this->session->userdata('karyawan_id') . "' GROUP BY client_wms_id)
									" . $perusahaan . "
									" . $divisi . "
									ORDER BY FORMAT(purchase_order.purchase_order_tgl_create,'dd-MM-yyyy') DESC");
	...
	public function GetPurchaseRequestHeaderById($id)
	{
		$query = $this->db->query("SELECT
                                po.*,
                                format(pr.purchase_request_tgl_dibutuhkan,'dd-MM-yyyy') as purchase_request_tgl_dibutuhkan2,
	...
	public function GetPurchaseRequestDetailById($id)
	{
        ...
        SELECT
                                                ...
                                                sku_barang.sku_barang_satuan,
                                                sku_barang.sku_barang_kemasan,
                                                ISNULL(purchase_order_detail.sku_barang_qty, 0) AS purchase_order_detail_qty,
                                                purchase_order_detail.purchase_order_id,
	...
	public function search_filter_chosen_sku($perusahaan, $sku_kode, $sku_nama_produk)
	{
