...
class M_Principle extends CI_Model
{
	...
	{
		// $this->db->select("sku_barang_supplier_id
		// 	,supplier_id,
		//  	sku_barang_id,
		//  	sku_barang_harga,
		// 	tgl_last_update,")
		// 	->from("sku_barang_supplier")
		// 	// ->where("principle_is_for_procurement", "1")
		// 	// ->where("principle_is_deleted", "0")
		// 	->order_by("supplier_id");
		$query = $this->db->query("select *,s.supplier_id as supplier_id_asli from supplier s left join sku_barang_supplier sb on s.supplier_id = sb.supplier_id ");
	...
	public function GetAllSupplier($arr)
	{
		// $this->db->select("sku_barang_supplier_id
		// 	,supplier_id,
		//  	sku_barang_id,
		...
		if ($arr == null || $arr == '') {
			$query = $this->db->select('*')->from('supplier s')->get();
		} else {

			$query = $this->db->select('*')->from('supplier s')
				->where_not_in('s.supplier_id', $arr)->get();
		}
	...
	public function Get_SupplierBySkuBarangId($skubarangid)
	{
		// $this->db->select("sku_barang_supplier_id
		// 	,supplier_id,
		//  	sku_barang_id,
		...
		// 	// ->where("principle_is_deleted", "0")
		// 	->order_by("supplier_id");
		// $query = $this->db->query("select * from supplier s left join sku_barang_supplier sb on s.supplier_id = sb.supplier_id where sb.sku_barang_id = '$skubarangid'");
		// $query = $this->db->query("select top 1 *,s.supplier_id as supplier_id_asli 
		// 		from supplier s 
		// 		left join sku_barang_supplier sbs on s.supplier_id = sbs.supplier_id and sbs.sku_barang_id = '$skubarangid'
		// 		left join sku_barang sb on sbs.sku_barang_id = sb.sku_barang_id 
		// ");
		$query = $this->db->query("select *,s.supplier_id as supplier_id_asli, format(tgl_last_update,'dd-MM-yyyy') as tglformat
				from supplier s
				left join sku_barang_supplier sbs on s.supplier_id = sbs.supplier_id where sbs.sku_barang_id = '$skubarangid'
		");
