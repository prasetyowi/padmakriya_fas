...
class M_Menu extends CI_Model
{
	...
	public function Get_Menu_Menu_Link_By_Menu_Kode( $menu_kode )
	{
		$query = $this->db->query("	select m.menu_link
									from menu_web as m
	...
	public function Get_Menu_Root_By_Menu_Application( $menu_application )
	{
		$query = $this->db->query("	select menu_id, menu_kode, menu_link, menu_name, menu_class, menu_parent, menu_c, menu_r, menu_u, menu_d, tipe, menu_application
									from menu_web as m
									where menu_application = '$menu_application' and isnull( menu_parent, '' ) = ''
									order by menu_kode");
	...
	public function Get_Menu_Child_By_Menu_Parent( $menu_kode )
	{
		$query = $this->db->query("	select menu_id, menu_kode, menu_link, menu_name, menu_class, menu_parent, menu_c, menu_r, menu_u, menu_d, tipe, menu_application
									from menu_web as m
									where menu_parent = '$menu_kode' and menu_r = 1
									union 
									select menu_id, menu_kode, menu_link, menu_name, menu_class, menu_parent, menu_c, menu_r, menu_u, menu_d, tipe, menu_application
									from menu_web as m
									where menu_parent in (	select menu_kode 
															from menu_web 
															where menu_parent = '$menu_kode') and menu_r = 1
									union 
									select menu_id, menu_kode, menu_link, menu_name, menu_class, menu_parent, menu_c, menu_r, menu_u, menu_d, tipe, menu_application
	...
	public function Get_Menu_Utility()
	{
		$query = $this->db->query("	select menu_kode, menu_c, menu_r, menu_u, menu_d, menu_name, menu_link, menu_parent, menu_order, menu_application
									from menu_web
									where menu_application = 'UTILITY'
									order by menu_kode" );
	...
	public function GetMenu_Sidebar($currentpage,$pengguna_grup_id,$menuparent="",$awalan=""){
		...
		$query = $this->db->query("	select 
										a.menu_id, cast(a.menu_kode as varchar(50)) as menu_kode, a.menu_link, a.menu_name, a.menu_class, a.menu_parent, a.menu_order, 	
										a.menu_c, a.menu_r, a.menu_u, a.menu_d, a.tipe, a.menu_application, a.menu_is_detail
									from menu_web a 
										inner join menu_access_web b on a.menu_kode = b.menu_kode and b.pengguna_grup_id = '$pengguna_grup_id' 
		...
		foreach($resultMenu as $rowMenu)
		{
			...
			if($menuparent == "")
			{
				...
				{
					$menu .= "<li ".(substr($currentpage,0,strlen($awalan.$menuname)) == $awalan.$menuname ?' class="active" ':'')."><a onclick='ShowStep(\"".$rowMenu['menu_kode']."\")'><i class=\"".$rowMenu["menu_class"]."\"></i> <label name='CAPTION-". $rowMenu["menu_kode"] ."'>".$rowMenu["menu_name"]."</label> <span class=\"fa fa-chevron-down\"></span></a>";
	...
	public function GetMenu_Sidebar_GlobalSetup($currentpage,$pengguna_grup_id,$menuparent="",$awalan=""){
		...
		foreach($resultMenu as $rowMenu)
		{
			...
			if($menuparent == "")
			{
				...
				{
					$menu .= "<li ".(substr($currentpage,0,strlen($awalan.$menuname)) == $awalan.$menuname ?' class="active" ':'')."><a onclick='ShowStep(\"".$rowMenu['menu_kode']."\")'><i class=\"".$rowMenu["menu_class"]."\"></i> ".$rowMenu["menu_name"]." <span class=\"fa fa-chevron-down\"></span></a>";
