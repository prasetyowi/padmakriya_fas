<script type="text/javascript">
    ...
    function Get_list_sku_back_order() {
        $.ajax({
            type: 'POST',
            url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/Get_list_sku_back_order') ?>",
            data: {
    ...
    function Get_list_karyawan() {
        ...
        $.ajax({
            type: 'POST',
            url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/Get_list_karyawan') ?>",
            data: {
                ...
                karyawan_manager_id: $("#SimulasiMpsTemp2-karyawan_manager_id").val(),
    ...
    function GetPrincipleByPerusahaan() {
        $.ajax({
            type: 'GET',
            url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/GetPrincipleByPerusahaan') ?>",
            data: {
                perusahaan: $("#AllotmentStockOrder-client_wms_id").val()
            },
            dataType: "JSON",
    ...
    function PilihKaryawan() {
        ...
        setTimeout(() => {
            arr_list_karyawan = [];
            Get_list_karyawan_supervisor_simulasi_mps($("#SimulasiMpsTemp2-bulan").val(), $("#SimulasiMpsTemp2-sku_id").val());
    ...
    function Get_list_allotment_stock_order_detail() {
        ...
        $.ajax({
            type: "POST",
            url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/Get_list_allotment_stock_order_detail') ?>",
            data: {
                ...
                $('#table_allotment_stock_order_detail').fadeOut("fast", function() {
                    ...
                    if (response.length > 0) {
                        ...
                        $.each(response, function(i, v) {
                            ...
                            if (v.is_berubah != "0") {
                                $("#table_allotment_stock_order_detail > tbody").append(`
                                    <tr style="background: #FF8787">
                                        ...
                                        <td class="text-center" style="text-align: center; vertical-align: middle;color:black;">${v.sku_qty_mng}</td>
                                        <td class="text-center" style="text-align: center; vertical-align: middle;color:black;">
                                            <button type="button" class="btn_tambah_jadwal_sku" style="border:none;background:transparent" onclick="ViewModalSimulasiSKU('${v.sku_id}','${v.sku_kode}','${v.sku_nama_produk}','${v.sku_kemasan}','${v.sku_satuan}','${v.sku_qty_mng}')"><i class="fas fa-eye text-primary" style="cursor: pointer"></i></button>
                                        </td>
                                    </tr>
                                `);

                            } else if (parseInt(v.sku_qty_sup) == 0) {
                                $("#table_allotment_stock_order_detail > tbody").append(`
                                    <tr style="background: #FF6868">
                                        ...
                                        <td class="text-center" style="text-align: center; vertical-align: middle;">${v.sku_kemasan}</td>
                                        <td class="text-center" style="text-align: center; vertical-align: middle;">${v.sku_satuan}</td>
                                        <td class="text-center" style="text-align: center; vertical-align: middle;">${v.sku_qty_mng}</td>
                                        <td class="text-center" style="text-align: center; vertical-align: middle;">
                                            <button type="button" class="btn_tambah_jadwal_sku" style="border:none;background:transparent" onclick="ViewModalSimulasiSKU('${v.sku_id}','${v.sku_kode}','${v.sku_nama_produk}','${v.sku_kemasan}','${v.sku_satuan}','${v.sku_qty_mng}')"><i class="fas fa-eye text-primary" style="cursor: pointer"></i></button>
                                        </td>
                                    </tr>
                            ...
                            } else if (parseInt(v.sku_qty_mng) != parseInt(v.sku_qty_sup)) {
                                $("#table_allotment_stock_order_detail > tbody").append(`
    ...
    function ViewModalBackOrder(client_pt_id, client_pt_nama) {
        ...
        $.ajax({
            type: "POST",
            url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/Get_list_back_order_by_customer') ?>",
            data: {
                tahun: $("#AllotmentStockOrder-allotment_stock_order_tahun").val(),
                perusahaan: $("#AllotmentStockOrder-client_wms_id").val(),
    ...
    function Get_proses_simulasi_mps() {
        ...
        $.ajax({
            type: "POST",
            url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/Get_proses_simulasi_mps') ?>",
            data: {
    ...
    function DeleteListKaryawanSupervisor(karyawan_supervisor_id, karyawan_manager_id, sku_id, tahun, bulan) {
        const findIndexData = arr_list_karyawan_supervisor_simulasi.findIndex((value) => value.karyawan_supervisor_id == karyawan_supervisor_id && value.karyawan_manager_id == karyawan_manager_id && value.sku_id == sku_id && value.tahun == tahun && value.bulan == bulan);
        ...
        if (findIndexData > -1) {
            arr_list_karyawan_supervisor_simulasi.splice(findIndexData, 1);

            if (arr_list_karyawan_supervisor_simulasi.length > 0) {
                $.each(arr_list_karyawan_supervisor_simulasi, function(i, v) {
                    if (v.bulan == bulan && v.tahun == tahun && v.sku_id == sku_id && v.karyawan_supervisor_id != karyawan_supervisor_id && v.karyawan_manager_id == karyawan_manager_id) {
    ...
    function DeleteAllKaryawan() {
        ...
        let arr_list_karyawan_supervisor_simulasi_temp = [];
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                ...
                arr_list_karyawan_supervisor_simulasi_temp = arr_list_karyawan_supervisor_simulasi;

                let findIndexData = arr_list_karyawan_supervisor_simulasi_temp
                    .map(function(value, index) {
                        return value.karyawan_manager_id == $("#SimulasiMpsTemp2-karyawan_manager_id").val() && value.sku_id == $("#SimulasiMpsTemp2-sku_id").val() && value.tahun == $("#SimulasiMpsTemp2-tahun").val() && value.bulan == $("#SimulasiMpsTemp2-bulan").val();
                    })
                ...
                $.each(findIndexData, function(i, v) {
                    if (v === true) {
                        arr_list_karyawan_supervisor_simulasi_temp[i] = "";
                    }
                })

                arr_list_karyawan_supervisor_simulasi = [];

                $.each(arr_list_karyawan_supervisor_simulasi_temp, function(i, v) {
                    if (v != "") {
                        arr_list_karyawan_supervisor_simulasi.push(v);
                    }
                })

                if (arr_list_karyawan_supervisor_simulasi.length > 0) {
                    $.each(arr_list_karyawan_supervisor_simulasi, function(i2, v2) {
        ...
        setTimeout(() => {
            ...
            if (cek_error == 0) {
                ...
                <?php if ($act == "edit") { ?>
                    $.ajax({
                        type: "POST",
                        url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/update_allotment_stock_order') ?>",
                        data: {
                            allotment_stock_order_id: $('#AllotmentStockOrder-allotment_stock_order_id').val(),
                            depo_id: "",
                            ...
                            karyawan_manager_simulasi: arr_list_karyawan_manager_simulasi,
                        ...
                        success: function(response) {
                            if (response.kode == 1) {
                                ...
                                setTimeout(() => {
                                    location.href = "<?= base_url() ?>FAS/Schedule/AllotmentStockOrderManager/AllotmentStockOrderManagerMenu";
                                }, 1000);
    ...
    function Get_list_karyawan_manager_simulasi_mps(bulan, sku_id) {
        ...
        $.ajax({
            type: "POST",
            url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/Get_list_karyawan_manager_simulasi_mps') ?>",
            data: {
                temp_id: $('#AllotmentStockOrder-allotment_stock_order_id').val(),
                tahun: $('#AllotmentStockOrder-allotment_stock_order_tahun').val(),
                bulan: bulan,
            ...
            success: function(response) {
                ...
                $("#SimulasiMpsTemp2-karyawan_manager_id").val('');
                $("#SimulasiMpsTemp2-karyawan_manager").val('');
                ...
                if (response.length > 0) {
                    ...
                    $.each(response, function(i, v) {
                        ...
                        if (arr_list_karyawan_supervisor_simulasi.length > 0) {
                            $.each(arr_list_karyawan_supervisor_simulasi, function(i2, v2) {
                                if (v.bulan == v2.bulan && v.tahun == v2.tahun && v.sku_id == v2.sku_id && v.karyawan_manager_id == v2.karyawan_manager_id) {
                        ...
                        $("#SimulasiMpsTemp2-karyawan_manager_id").val(v.karyawan_manager_id);
                        $("#SimulasiMpsTemp2-karyawan_manager").val(v.karyawan_nama);
    ...
    function Get_list_karyawan_supervisor_simulasi_mps(bulan, sku_id) {
        ...
        $("#table_list_karyawan_supervisor_simulasi_mps > tbody").html('');
        $("#table_list_karyawan_supervisor_simulasi_mps > tbody").empty();

        $.ajax({
            type: "POST",
            url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/Get_list_karyawan_supervisor_simulasi_mps') ?>",
            data: {
                temp_id: $('#AllotmentStockOrder-allotment_stock_order_id').val(),
                tahun: $('#AllotmentStockOrder-allotment_stock_order_tahun').val(),
                bulan: bulan,
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    ...
                    $.each(response, function(i, v) {
                        ...
                        $("#table_list_karyawan_supervisor_simulasi_mps > tbody").append(`
                            <tr>
                                <td class="text-center" style="text-align: center; vertical-align: middle;">
                                    ${i+1}
                                    <input type="hidden" class="form-control" name="item-${i}-back_order_simulasi_mps-karyawan_supervisor_id" id="item-${i}-back_order_simulasi_mps-karyawan_supervisor_id" value="${v.karyawan_supervisor_id}" onchange="UpdateKaryawanSupervisorSimulasiMPS('${i}','${v.karyawan_supervisor_id}','${v.karyawan_manager_id}','${v.sku_id}','${v.tahun}','${v.bulan}','${v.sku_qty}')" />
                                </td>
                                ...
                                <td class="text-center" style="text-align: center; vertical-align: middle;">
                                    <input type="text" class="form-control" name="item-${i}-back_order_simulasi_mps-sku_qty" id="item-${i}-back_order_simulasi_mps-sku_qty" value="${v.sku_qty}" onchange="UpdateKaryawanSupervisorSimulasiMPS('${i}','${v.karyawan_supervisor_id}','${v.karyawan_manager_id}','${v.sku_id}','${v.tahun}','${v.bulan}','${v.sku_qty}')" />
                                </td>
                                <td class="text-center" style="text-align: center; vertical-align: middle;">
                                    <button type="button" class="btn_delete_jadwal_sku" style="border:none;background:transparent" onclick="DeleteListKaryawanSupervisor('${v.karyawan_supervisor_id}','${v.karyawan_manager_id}','${v.sku_id}','${v.tahun}','${v.bulan}')"><i class="fas fa-trash text-danger" style="cursor: pointer"></i></button>
                                </td>
                            </tr>
                        `);
                    });
    ...
    function UpdateKaryawanSupervisorSimulasiMPS(index, karyawan_supervisor_id, karyawan_manager_id, sku_id, tahun, bulan, sku_qty_asli) {
        const findIndexData = arr_list_karyawan_supervisor_simulasi.findIndex((value) => value.karyawan_supervisor_id == karyawan_supervisor_id && value.sku_id == sku_id && value.tahun == tahun && value.bulan == bulan);
        ...
        if (findIndexData > -1) {
            ...
            if (sku_qty > sku_qty_max - total_sku_qty_per_bulan) {
                ...
                arr_list_karyawan_supervisor_simulasi[findIndexData]['sku_qty'] = sku_qty_asli;
                ...
                arr_list_karyawan_supervisor_simulasi[findIndexData]['sku_qty'] = sku_qty;
    ...
    function CekQtySisa() {
        ...
        $.ajax({
            type: "GET",
            url: "<?= base_url('FAS/Schedule/AllotmentStockOrderManager/Get_total_qty_simulasi_mps_temp2_not_in_bulan') ?>",
            data: {
                allotment_stock_order_id: $('#AllotmentStockOrder-allotment_stock_order_id').val(),
                tahun: $('#AllotmentStockOrder-allotment_stock_order_tahun').val(),
                bulan: $("#SimulasiMpsTemp2-bulan").val(),
