<script>
	...
	function getDataSearch() {
		...
		$.ajax({
			async: false,
			type: 'POST',
			url: "<?= base_url('FAS/Pengaturan/PengaturanApproval/checkData') ?>",
			data: {
				client_wms_id: $('#filter_perusahaan option:selected').val()
			},
			dataType: "json",
			success: function(response) {
				...
				if (response == 1 || response == 2) {
					$.ajax({
						async: false,
						type: 'POST',
						url: "<?= base_url('FAS/Pengaturan/PengaturanApproval/getDataSearch') ?>",
						data: {
								...
								$.each(data, function() {
									...
									<tr>
										...
										<td style='vertical-align:middle;text-align: center; ' >
											<a class="btn btn-primary" tittle="lihat"  href="<?php echo site_url('FAS/Pengaturan/PengaturanApproval/PengaturanApprovalView?id=') ?>${this.approval_setting_id}"><i class="fas fa-eye"></i></a>
											<a class="btn btn-warning" tittle="Edit"  href="<?php echo site_url('FAS/Pengaturan/PengaturanApproval/PengaturanApprovalEdit?id=') ?>${this.approval_setting_id}"><i class="fas fa-edit"></i></a>
										</td>
									</tr>
						`);
									no++;
