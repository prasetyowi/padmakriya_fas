<script>
	...
	function getListKaryawanDivisi() {
		...
		$.ajax({
			type: 'GET',
			url: "<?= base_url('FAS/Pengaturan/PengaturanApproval/getListKaryawanDivisi') ?>",
	...
	function getListKaryawanLevel() {
		...
		$.ajax({
			type: 'GET',
			url: "<?= base_url('FAS/Pengaturan/PengaturanApproval/getListKaryawanLevel') ?>",
			success: function(response) {
				let data = response;
				data.forEach(function(item, index) {
					arrSlcLevel.push({
	...
	function getListKaryawanLevelByDivisi(index) {
		...
		$.ajax({
			type: 'GET',
			url: "<?= base_url('FAS/Pengaturan/PengaturanApproval/getListKaryawanLevelByDivisi') ?>",
			data: {
				divisi_id: divisi_id
			},
			async: false,
	...
	function getListNamaGroupApproval() {
		...
		$.ajax({
			async: false,
			type: 'GET',
			url: "<?= base_url('FAS/Pengaturan/PengaturanApproval/getListNamaGroupApproval') ?>",
			dataType: 'json',
			success: function(response) {
				let data = response;
				if (data)
	...
	//     $.ajax({
	//         type: 'GET',
	//         url: "<?= base_url('FAS/Pengaturan/PengaturanApproval/CheckParameterTersimpan') ?>",
	//         data: {
	//             parameter: parameter
	//         },
	//         success: function(response) {
	...
	$("#tableDetail").on("click", ".infoDetail", function() {
		...
		if (id_approval_group == '') {
			...
			$.ajax({
				type: 'POST',
				url: "<?= base_url('FAS/Pengaturan/PengaturanApproval/getDataByApprovalGroupId') ?>",
				data: {
	...
	$('#saveData').click(function(e) {
		...
		// let slc_divisi = $("[name='slc_divisi[]']").map(function() {
		// 	return this.value
		// }).get();
		...
		Swal.fire({
			...
			if (result.value == true) {
				$.ajax({
					url: "<?= base_url('FAS/Pengaturan/PengaturanApproval/UpdatePengaturanApproval') ?>",
					type: 'POST',
					data: {
						id: add_id,
						is_paralel: add_is_paralel,
						parameter: add_parameter,
						perusahaan: add_perusahaan,
						menu_id: add_menu,
						...
						if (response.status == 1) {
							...
							setTimeout(function() {
								window.location.href =
									"<?= base_url('FAS/Pengaturan/PengaturanApproval/PengaturanApprovalMenu') ?>";
							}, 3000);
						} else {
							Swal.fire(
								'Error!',
	...
	function getDataApproval() {
		...
		$.ajax({
			type: 'GET',
			url: "<?= base_url() ?>FAS/Pengaturan/PengaturanApproval/getApprovalByID",
			dataType: "JSON",
			data: {
				id: $("#add_id").val()
			},
			success: function(response) {
				$.each(response, function(i, v) {
					...
					arrApprovalGroup.forEach(function(item, index) {
						...
						option_approval_group = option_approval_group +
							'<option value="' + item.approval_group_id + '" data-kode="' + item
							.approval_group_nama + '" ' + selected + '>' + item
							.approval_group_nama +
							'</option>';
						'</option>';
