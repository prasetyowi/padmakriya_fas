<script text="text/javascript">
	...
	$(document).on('click', '#btn-search-data-pn', function() {
		...
		$.ajax({
			type: 'POST',
			url: "<?= base_url('FAS/Pengadaan/PenerimaanBarangPO/search_penerimaan_by_filter') ?>",
			data: {
			...
			success: function(response) {
				...
				if (response != 0) {
					$.each(response, function(i, v) {
						if (v.penerimaan_sku_barang_status == 'In Progress Receiving') {
							...
							<tr>
								...
								<td class="text-center">${v.penerimaan_sku_barang_status}</td>
								<td class="text-center"><a href="<?= base_url() ?>FAS/Pengadaan/PenerimaanBarangPO/edit/?id=${v.purchase_order_id}&pnid=${v.penerimaan_sku_barang_id}" class="btn btn-primary btn-small btn-delete-sku"><i class="fa fa-pencil"></i></a></td>
							</tr>
						`);
						}
						if (v.penerimaan_sku_barang_status == 'Completed') {
							$("#table_list_data_pn > tbody").append(`
                                <tr>
                                    <td class="text-center">${v.penerimaan_sku_barang_kode}</td>
                                    <td class="text-center">${v.purchase_order_kode}</td>
                                    ...
                                    <td class="text-center">${v.penerimaan_sku_barang_who_create}</td>
                                    <td class="text-center">${v.penerimaan_sku_barang_status}</td>
                                    <td class="text-center"><a href="<?= base_url() ?>FAS/Pengadaan/PenerimaanBarangPO/view/?id=${v.purchase_order_id}&pnid=${v.penerimaan_sku_barang_id}" class="btn btn-warning btn-small btn-delete-sku"><i class="fa fa-search"></i></a></td>
                                </tr>
                            `);
						}
						if (v.penerimaan_sku_barang_status == 'Partially Received') {
	...
	$(document).on('change', '#penerimaanorder-purchase_order_kode', function() {
		...
		$('#header').fadeOut("slow", function() {
			...
			$.ajax({
				...
				url: "<?= base_url('FAS/Pengadaan/PenerimaanBarangPO/GetDataDetailByKodePo') ?>",
				dataType: "json",
				// data: "PrincipleId=" + id,
				success: function(response) {
					if (response) {
	...
	function AppendToTableDetail() {
		...
		$('#tablebarangpo').fadeOut("slow", function() {
			...
			$.ajax({
				url: "<?= base_url('FAS/Pengadaan/PenerimaanBarangPO/GetPurchaseRequestDetailById'); ?>",
				type: "POST",
				data: {
					po_id: $('#hpo_id').val()
				},
				...
				success: function(data) {
					...
					<tr id="row-${i}">
                        ...
                        <td style="display:none" ><input type="number" class="form-control numeric" id="item-${i}-sku_barang_harga" ${v.purchase_order_detail_qty_sisa==0? 'disabled' : ''} onchange="handleChecKodeByEnter(event,this.value)" value="${Math.round(v.sku_barang_harga)}"></td>
	...
	$(document).on('click', '#btnsavepenerimaanpo', function() {
		...
		let kode_po = $('#penerimaanorder-purchase_order_kode option:selected').val();
		...
		Swal.fire({
			...
			if (result.value == true) {
				...
				$.ajax({
					async: false,
					url: "<?= base_url('FAS/Pengadaan/PenerimaanBarangPO/insert_penerimaan_barangpo'); ?>",
					type: "POST",
					data: {
						po_id: $('#hpo_id').val(),
					...
					success: function(data) {
						if (data == 1) {
							...
							setTimeout(() => {
								location.href = "<?= base_url(); ?>FAS/Pengadaan/PenerimaanBarangPO/PenerimaanBarangPOMenu";
							}, 500);
						} else {
	...
	$(document).on('click', '#btnkonfirmasipenerimaanpo', function() {
		...
		let penerimaan_order_tgl = $('#penerimaanorder-penerimaan_order_tgl').val();
		let penerimaan_order_tgl_dibutuhkan = $('#penerimaanorder-penerimaan_order_tgl_dibutuhkan').val();
		...
		Swal.fire({
			...
			if (result.value == true) {
				...
				$.ajax({
					async: false,
					url: "<?= base_url('FAS/Pengadaan/PenerimaanBarangPO/konfirmasi_penerimaan_barangpo'); ?>",
					type: "POST",
					data: {
						po_id: $('#hpo_id').val(),
						pr_id: $('#hpr_id').val(),
						tipe_pengadaan_id: $('#penerimaanorder-tipe_pengadaan_id option:selected').val(),
						tipe_transaksi_id: $('#penerimaanorder-tipe_transaksi_id option:selected').val(),
						client_wms_id: $('#penerimaanorder-client_wms_id option:selected').val(),
						po_kode: $('#penerimaanorder-purchase_order_kode option:selected').val(),
	...
	$(document).on('click', '#btneditpenerimaanpo', function() {
		...
		let kode_po = $('#editpenerimaanorder-purchase_order_kode option:selected').val();
		let count = arrcountitempo[0];
		if (kode_po == null || kode_po == '') {
			message("Error!", "Mohon Pilih Kode PO", "error");
			return false;
		...
		if ($('#editpenerimaanorder-client_wms_id option:selected').val() == '' || $('#editpenerimaanorder-client_wms_id option:selected').val() == null) {
		...
		Swal.fire({
			...
			if (result.value == true) {
				...
				$.ajax({
					async: false,
					url: "<?= base_url('FAS/Pengadaan/PenerimaanBarangPO/update_penerimaan_barangpo'); ?>",
					type: "POST",
					data: {
						po_id: $('#hpo_id').val(),
	...
	$(document).on('click', '#btneditkonfirmasipenerimaanpo', function() {
		...
		Swal.fire({
			...
			if (result.value == true) {
				...
				$.ajax({
					async: false,
					url: "<?= base_url('FAS/Pengadaan/PenerimaanBarangPO/konfirmasi_update_penerimaan_barangpo'); ?>",
					type: "POST",
					data: {
						po_id: $('#hpo_id').val(),
						client_wms_id: $('#editpenerimaanorder-client_wms_id option:selected').val(),
