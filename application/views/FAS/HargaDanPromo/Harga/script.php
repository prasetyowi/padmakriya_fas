<script type="text/javascript">
	...
	$("#btn-search-data-harga").click(function() {

		$("#loading").show();
		$("#btn-search-data-harga").prop("disabled", true);

		$.ajax({
			type: 'GET',
			url: "<?= base_url('FAS/MasterPricing/HargaDanPromo/Harga/search_sku_harga_by_filter') ?>",
			data: {
				sku_harga_kode: $("#Filter-sku_harga_kode").val(),
				client_wms_id: $("#Filter-client_wms_id").val(),
				sku_harga_is_aktif: $("#Filter-sku_harga_is_aktif").val()
			...
			success: function(response) {
				...
				$("#loading").hide();
				$("#btn-search-data-harga").prop("disabled", false);

				$('#table-data-harga > tbody').empty('');

				if ($.fn.DataTable.isDataTable('#table-data-harga')) {
					$('#table-data-harga').DataTable().clear();
					$('#table-data-harga').DataTable().destroy();
				}

				if (response != 0) {
					$.each(response, function(i, v) {

						if (v.sku_harga_status == "Draft") {

							$("#table-data-harga > tbody").append(`
                                <tr id="row-${i}">
                                    <td class="text-center">${i+1}</td>
                                    <td class="text-center">${v.sku_harga_kode}</td>
                                    <td class="text-center">${v.client_wms_nama}</td>
                                    <td class="text-center">${v.sku_harga_keterangan}</td>
									<td class="text-center"><span id="item-${i}-harga-lokasi"></span></td>
                                    <td class="text-center">${v.sku_harga_tgl_berlaku_awal}</td>
                                    <td class="text-center">${v.sku_harga_tgl_berlaku_akhir}</td>
                                    <td class="text-center">${v.sku_harga_tgl_create}</td>
                                    <td class="text-center">${v.sku_harga_who_create}</td>
                                    <td class="text-center">${v.sku_harga_tgl_approved}</td>
                                    <td class="text-center">${v.sku_harga_who_aprroved}</td>
                                    <td class="text-center">${v.sku_harga_status}</td>
                                    <td class="text-center ${v.sku_harga_is_aktif == '1' ? 'text-success' : 'text-danger'}">${v.sku_harga_is_aktif == '1' ? 'Active' : 'Not Active'}</td>
                                    <td class="text-center">
                                        <a href="<?= base_url() ?>FAS/MasterPricing/HargaDanPromo/Harga/edit/?id=${v.sku_harga_id}" target="_blank" class="btn btn-info btn-small"><i class="fa fa-pencil"></i></a>
                                    </td>
                                </tr>
                            `);

						} else if (v.sku_harga_status == "In Progress Approval") {
						...
						} else if (v.sku_harga_status == "Approved") {
							...
							$("#table-data-harga > tbody").append(`
                                <tr id="row-${i}">
                                    ...
                                    <td class="text-center ${v.sku_harga_is_aktif == '1' ? 'text-success' : 'text-danger'}">${v.sku_harga_is_aktif == '1' ? 'Active' : 'Not Active'}</td>
                                    <td class="text-center">
                                        <a href="<?= base_url() ?>FAS/MasterPricing/HargaDanPromo/Harga/detail/?id=${v.sku_harga_id}" target="_blank" class="btn btn-info btn-small"><i class="fa fa-search"></i></a>
                                        <a href="<?= base_url() ?>FAS/MasterPricing/HargaDanPromo/Harga/duplicate/?id=${v.sku_harga_id}" target="_blank" class="btn btn-danger btn-small"><i class="fa fa-copy"></i></a>
                                    </td>
                                </tr>
                            `);
						}
						...
						$.ajax({
							async: false,
							type: 'GET',
							url: "<?= base_url('FAS/MasterPricing/HargaDanPromo/Harga/GetHargaDepo') ?>",
							data: {
							...
							success: function(response2) {
								...
								if (response2 != 0) {
									...
									$("#item-" + i + "-harga-lokasi").append(arr_lokasi.toString());
								}
							}
						});
					});

					$('#table-data-harga').DataTable();
				}
			}
		});
			...
			if (arr_sku_selected[i] != "") {
				$("#table-input-harga > tbody").append(`
					<tr id="row-${i}">
						...
						<td class="text-center">
							<input type="number" id="item-${i}-SKUHargaDetail-sku_qty" class="form-control" value="1" />
						</td>
						<td class="text-center">
							<span class="sku-satuan-label">${v.sku_satuan}</span>
						</td>
						<td class="text-center">
							<input type="checkbox" id="item-${i}-SKUHargaDetail-sku_with_tax" name="CheckboxWithTax" value="1" />
						</td>
						<td class="text-center">
							<input type="text" id="item-${i}-SKUHargaDetail-sku_nominal_harga" class="form-control" value="0" />
						</td>
						<td class="text-center">
	...
	function GetSelectedSKU() {
		...
		$.ajax({
			type: 'POST',
			url: "<?= base_url('FAS/MasterPricing/HargaDanPromo/Harga/GetSelectedSKU') ?>",
			data: {
				sku_id: arr_sku
			},
			dataType: "JSON",
