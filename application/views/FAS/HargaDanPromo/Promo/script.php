<script type="text/javascript">
	...
	$('#select-sku-detail').click(function(event) {
		if (this.checked) {
			// Iterate each checkbox
			$('[name="CheckboxSKUDetail1"]:checkbox:not(:disabled)').each(function() {
				this.checked = true;
			});
		} else {
			$('[name="CheckboxSKUDetail1"]:checkbox:not(:disabled)').each(function() {
	...
	function AturBonus(sku_promo_detail2_id, sku_promo_detail1_id, sku_promo_id, idx, idx2) {
		...
		if (principle_id != "") {
			...
			if (kategori_id != "") {
				...
				$("#SKUPromoDetail-kategori_id_by_one").val('');
	...
	function AturDiskon(sku_promo_detail2_id, sku_promo_detail1_id, sku_promo_id, idx, idx2) {
		...
		if (principle_id != "") {
			...
			if (kategori_id != "") {
				...
				$("#FilterPengaturanDiskon-kategori_id_by_one").val('');
				...
				$("#FilterPengaturanDiskon-kategori_id_by_one").val(kategori_id);
				$("#SKUPromoDetail2Diskon-check_pilihan").val('by_one');
	...
	function AturBonusBySKU(sku_promo_detail2_id, sku_promo_detail1_id, sku_promo_id, idx, idx2, sku_id, sku_kode, sku_nama_produk, sku_kemasan, sku_satuan) {
		...
		if (principle_id != "") {
			...
			if (sku_kode != "") {
				...
				$("#panel-pengaturan-detail-by-sku").hide();

				var checked = $(`#SKUPromoDetail-sku_promo_detail1_use_value_order_${idx}`).prop('checked');

				if (checked) {
					$("#dasar_jumlah_order").val('value order')
					$("#thMinimumBySKU").html("<span name='CAPTION-VALUEMININUM'>Value Minimum</span>")
				} else {
					$("#dasar_jumlah_order").val('qty order')
					$("#thMinimumBySKU").html("<span name='CAPTION-QTYMINIMUM'>Qty Minimum</span>")
				}
	...
	function AddSKUPromoDetail2(idx) {
		...
		if (principle_id !== "") {
			...
			if (use_group == "1") {
				...
				var msg = GetLanguageByKode('CAPTION-ALERT-PILIHGROUPSKU');
				message_custom("Error", "error", msg);
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response.data != 0) {
												...
												<div class="col-xs-4">
													<table class="table table-bordered" id="table-syarat-item-${i}" style="width:100%;height: 250px;">
														...
														<tbody>
															<tr>
																<td colspan="2">
																	<input type="checkbox" onchange="chgQtyOrder(this.checked, '${i}')" name="SKUPromoDetail[sku_promo_detail1_use_qty_order_${i}]" id="SKUPromoDetail-sku_promo_detail1_use_qty_order_${i}" value="1" onChange="UpdateSKUDetailPromo1('${i}')" ${v.sku_promo_detail1_use_qty_order == '1'?'checked':''}> Dasar Jumlah Qty Order
																</td>
															</tr>
															<tr>
																<td colspan="2">
																	<input type="checkbox" onchange="chgValueOrder(this.checked, '${i}')" name="SKUPromoDetail[sku_promo_detail1_use_value_order_${i}]" id="SKUPromoDetail-sku_promo_detail1_use_value_order_${i}" value="1" onChange="UpdateSKUDetailPromo1('${i}')" ${v.sku_promo_detail1_use_value_order == '1'?'checked':''}> Dasar Jumlah Value Order
																</td>
															</tr>
															<tr>
																<td colspan="2">
																	...
																	<select class="form-control" name="SKUPromoDetail[sku_promo_detail1_min_order_sku_${i}]" id="SKUPromoDetail-sku_promo_detail1_min_order_sku_${i}" style="width:20%;" onChange="UpdateSKUDetailPromo1('${i}')">
																		<option value="1" ${v.sku_promo_detail1_min_order_sku == '1'?'selected':''}>Aktif</option>
																		<option value="0" ${v.sku_promo_detail1_min_order_sku == '1'?'selected':''}>Tidak Aktif</option>
																	</select>
																</td>
															</tr>
															<tr>
												...
												<div class="col-xs-4">
													<table class="table table-bordered" id="table-syarat-item-${i}" style="width:100%;height: 250px;">
														...
														<tbody>
															<tr>
																<td colspan="2">
																	<input type="checkbox" onchange="chgQtyOrder(this.checked, '${i}')" name="SKUPromoDetail[sku_promo_detail1_use_qty_order_${i}]" id="SKUPromoDetail-sku_promo_detail1_use_qty_order_${i}" value="1" onChange="UpdateSKUDetailPromo1('${i}')"> Dasar Jumlah Qty Order
																</td>
															</tr>
															<tr>
																<td colspan="2">
																	<input type="checkbox" onchange="chgValueOrder(this.checked, '${i}')" name="SKUPromoDetail[sku_promo_detail1_use_value_order_${i}]" id="SKUPromoDetail-sku_promo_detail1_use_value_order_${i}" value="1" onChange="UpdateSKUDetailPromo1('${i}')"> Dasar Jumlah Value Order
																</td>
															</tr>
															<tr>
																<td colspan="2">
					...
					$("#headerPerusahaan").prop("disabled", false);
				}
			}
		});
	}
	...
	$("#btn_search_sku_by_sku").click(function() {
		...
		<?php foreach ($Principle as $row) : ?>
			$("#FilterPencarianSKU-principle_id").append(`<option value="<?= $row['principle_id'] ?>" ${ principle_id == '<?= $row['principle_id'] ?>' ? 'selected' : ''}><?= $row['principle_kode'] ?></option>`);
		<?php endforeach; ?>
		$("#FilterPencarianSKU-principle_id").prop('disabled', true);
	...
	function GetPromoDetail2Bonus() {
		$.ajax({
			...
			success: function(response) {
				...
				$.each(response, function(i, v) {
					if (stringValue == 'value order') {
						str = `<input type="text" onkeyup="formatNominal(this)" id="item-${i}-SKUPromoDetail2Bonus-sku_min_qty" class="form-control input-sm" value="${v.sku_min_value == 0 ? '' : formatNumber(parseFloat(v.sku_min_value))}" onChange="UpdateSKUPromoDetail2Bonus('${i}','all')" />`
					} else {
						str = `<input type="number" id="item-${i}-SKUPromoDetail2Bonus-sku_min_qty" class="form-control input-sm" value="${v.sku_min_qty}" onChange="UpdateSKUPromoDetail2Bonus('${i}','all')" />`
					}
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response != "0") {
					...
					$.each(response, function(i, v) {
						if (stringValue == 'value order') {
							str = `<input type="text" onkeyup="formatNominal(this)" id="item-${i}-SKUPromoDetail2Bonus-sku_min_qty_by_sku" class="form-control input-sm" value="${v.sku_min_value == 0 ? '' : formatNumber(parseFloat(v.sku_min_value))}" onChange="UpdateSKUPromoDetail2Bonus('${i}','by_sku')" />`
						} else {
							str = `<input type="number" id="item-${i}-SKUPromoDetail2Bonus-sku_min_qty_by_sku" class="form-control input-sm" value="${v.sku_min_qty}" onChange="UpdateSKUPromoDetail2Bonus('${i}','by_sku')" />`
						}
	...
	$(document).on("click", ".btn-choose-sku-multi", function() {
		...
		var numberOfChecked = $('input[name="CheckboxSKU"]:checked:not(:disabled)').length;
		var no = 1;
		var check_pilihan = $("#SKUPromoDetail2Bonus-check_pilihan").val();
		...
		if (numberOfChecked > 0) {
			...
			for (var i = 0; i < jumlah; i++) {
				var checked = $('[id="check-sku-' + i + '"]:checked:not(:disabled)').length;
	...
	$("#btn_cari_sku").click(function() {
		...
		if ($("#FilterPencarianSKU-principle_id").val() != "") {
			...
			if (check_pilihan == "all") {
				...
				$.ajax({
					...
					data: {
						...
						sku_group_id: "",
						sku_group_nama: "",
						perusahaan: $("#FilterPencarianSKU-client_wms_id").val()
					},
					dataType: 'JSON',
					success: function(response) {
						if (response != 0) {
	...
	$("#btn_cari_sku_2").click(function() {
		...
		if ($("#FilterPencarianSKUDetail1-principle_id").val() != "") {
			...
			$.ajax({
				...
				data: {
					...
					sku_group_id: "",
					sku_group_nama: "",
					perusahaan: $("#FilterPencarianSKUDetail1-client_wms_id").val()
				},
				dataType: 'JSON',
				success: function(response) {
					if (response != 0) {
	...
	$("#btn_cari_sku_pengecualian").click(function() {
		...
		if ($("#FilterPencarianSKUPengecualian-principle_id").val() != "") {
			...
			$.ajax({
				...
				data: {
					...
					perusahaan: $("#FilterPencarianSKUPengecualian-client_wms_id").val()
				},
				dataType: 'JSON',
				success: function(response) {
					if (response != 0) {
	...
	$("#btn_save_pengaturan_bonus2").click(function() {
		...
		if (arr_pengaturan_header.length > 0) {
			$.each(arr_pengaturan_header, function(i, v) {
				...
				if (v.sku_min_qty != '') {
					...
					console.log(cek_sku_min_qty, 'aaa');
					cek_sku_min_qty++;
					return false;
				}
		...
		console.log(cek_sku_min_qty_before, cek_sku_min_qty, cek_qty_bonus, cek_error);
	...
	$("#btn_save_pengaturan_bonus").click(function() {
		...
		$("#table-pengaturan-header > tbody tr").each(function(idx) {
			...
			if ($("#item-" + idx + "-SKUPromoDetail2Bonus-sku_min_qty").val() != "" || $("#item-" + idx + "-SKUPromoDetail2Bonus-sku_min_qty").val() != "0") {
				if (parseInt($("#item-" + idx + "-SKUPromoDetail2Bonus-sku_min_qty").val()) > parseInt(sku_min_qty_before)) {
	...
	function initDataDepo() {
		...
		$.ajax({
			...
			success: function(response) {
				...
				$('#SKUPromo-depo_id').selectpicker('refresh');
			}
		});
	}
