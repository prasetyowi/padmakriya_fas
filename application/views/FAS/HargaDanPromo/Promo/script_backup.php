<script type="text/javascript">
	...
	$('#select-sku-detail').click(function(event) {
		if (this.checked) {
			// Iterate each checkbox
			$('[name="CheckboxSKUDetail"]:checkbox').each(function() {
				this.checked = true;
			});
		} else {
			$('[name="CheckboxSKUDetail"]:checkbox').each(function() {
			...
			$.ajax({
				...
				success: function(response) {
					...
					if (response != 0) {
							...
							<tr id="row-${i}">
								...
								<td class="text-center" style="width:10%;">
                                    <select id="item-${i}-SKUPromoDetail2BonusDetail-referensi_diskon_id" class="form-control select2"  onChange="UpdateSKUPromoDetail2BonusDetail('${v.sku_promo_detail2_bonus2_id}','${v.sku_promo_detail2_bonus_id}','${v.sku_promo_detail2_id}','${v.sku_promo_detail1_id}','${v.sku_promo_id}','${v.sku_id}','${i}','all')">
										...
										<?php foreach ($ReferensiDiskon as $value) : ?>
						...
						$("item-" + i + "-SKUPromoDetail2BonusDetail-referensi_diskon_id").html('');

						$.ajax({
							type: 'GET',
							url: "<?= base_url('FAS/MasterPricing/HargaDanPromo/Promo/GetReferensiDiskon') ?>",
							dataType: "JSON",
							success: function(response2) {
								if (response2 != 0) {
									$.each(response2, function(idx, val) {
										$("item-" + i + "-SKUPromoDetail2BonusDetail-referensi_diskon_id").append(`<option value="${val.referensi_diskon_id}" ${val.referensi_diskon_id == v.referensi_diskon_id}>${val.referensi_diskon_kode}</option>`);
									})
								}
					...
					success: function(response) {
						...
						if (response.length > 0) {
							...
							<tr id="row-${i}">
								...
								<td style="vertical-align:middle; text-align:center;">
									<select name="FilterPencarianSKU[principle_id]" class="input-sm form-control select2" id="item-${idx}-${i}-SKUPromoDetail2-kategori_grup" onchange="GetKategoriByGroup('${v.sku_promo_detail2_id}',this.value,'${idx}','${i}')">
										<option value="">** <span name="CAPTION-PILIH">Pilih</span> **</option>
	...
	function GetKategoriByGroup(sku_promo_detail2_id, group, idx, idx2) {
		// alert(idx)
		$.ajax({
			async: false,
			type: 'GET',
									...
									if (response.length > 0) {
										$.each(response, function(idx, val) {
											...
											<tr id="row-${idx}">
												...
												<td style="vertical-align:middle; text-align:center;">
													<select name="FilterPencarianSKU[principle_id]" class="input-sm form-control select2" id="item-${i}-${idx}-SKUPromoDetail2-kategori_grup" onchange="GetKategoriByGroup('${val.sku_promo_detail2_id}',this.value,'${i}','${idx}')">
														<option value="">** <span name="CAPTION-PILIH">Pilih</span> **</option>
														<?php foreach ($KategoriGroup as $row) : ?>
	...
	function UpdateSKUPromoDetail2Bonus(idx, act) {
		...
		var is_berkelipatan = $("#item-" + idx + "-SKUPromoDetail2Bonus-is_berkelipatan").val();
		...
		if (act == "all") {
			...
			is_berkelipatan = $("#item-" + idx + "-SKUPromoDetail2Bonus-is_berkelipatan").val();
			...
			is_berkelipatan = $("#item-" + idx + "-SKUPromoDetail2Bonus-is_berkelipatan_by_one").val();
			sku_promo_detail2_bonus_nourut = $("#item-" + idx + "-SKUPromoDetail2Bonus-sku_promo_detail2_bonus_nourut").val();
			...
			is_berkelipatan = $("#item-" + idx + "-SKUPromoDetail2Bonus-is_berkelipatan_by_sku").val();
			sku_promo_detail2_bonus_nourut = $("#item-" + idx + "-SKUPromoDetail2Bonus-sku_promo_detail2_bonus_nourut").val();
		}
		...
		$.ajax({
			...
			data: {
				...
				is_berkelipatan: is_berkelipatan,
				...
				if (response != "0") {
					...
					$.each(response, function(i, v) {
						...
						<tr>
							...
							<td class="text-center">
								<input type="number" id="item-${i}-SKUPromoDetail2Bonus-sku_min_qty_by_one" class="form-control input-sm" value="${v.sku_min_qty}" onChange="UpdateSKUPromoDetail2Bonus('${i}','by_one')" />
