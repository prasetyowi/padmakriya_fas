<script>
	...
	function getKreditLimitDetailByID() {
		// add principal pelanggan
		let kredit_limit_id = $('#add_id_kredit').val();
		$.ajax({
			type: 'GET',
			url: "<?= base_url('FAS/ManagementPelanggan/PengaturanKreditLimit/getKreditLimitDetailByID') ?>",
			async: false,
			data: {
				kredit_limit_id: kredit_limit_id
			},
			success: function(response) {
				let data = response;
				data.forEach(function(item, index) {
					arrDetail.push({
						principle_id: item.principle_id,
						principle_kode: item.principle_kode,
						top: item.pengajuan_kredit_top,
						is_kredit: item.is_kredit,
						kredit_limit: parseInt(item.pengajuan_kredit_limit ==
							null || item.pengajuan_kredit_limit ==
							'.0000' ? 0 : item.pengajuan_kredit_limit),
						max_invoice: item.pengajuan_kredit_maks_invoice == null ? 0 : item
							.pengajuan_kredit_maks_invoice,
						segment_harga: null,
						segment1: item.client_pt_segmen_id1,
						segment2: item.client_pt_segmen_id2,
						segment3: item.client_pt_segmen_id3,
						segment1_nama: item.nama_segmen1,
						segment2_nama: item.nama_segmen2,
						segment3_nama: item.nama_segmen3,
						is_alamat_beda: item.pengajuan_kredit_is_alamat_penagihan_beda ==
							null ? 0 : item.pengajuan_kredit_is_alamat_penagihan_beda,
						alamat_penagihan_id: item.client_pt_id_penagihan,
	...
	function showTableDetailDraft() {
		...
		let option_segment2 = '';
		let option_segment3 = '';
		//select segment berdasarkan defaukt segment di client pt
		...
		//append data draft detail ke view table
		arrDraftDetail.forEach(function(item, index) {
			$('#tableAddDetail tbody').append(`
            					<tr id="tr-${item.principle_kode}">
                                    ...
                                    <td style="text-align: center;">
										<input type="number" class="form-control" name="add_detail_max_invoice[]" id="add_detail_max_invoice-${index}" onkeypress="return event.charCode >= 48 && event.charCode <= 57" value="0">
									</td>
                                    <td style="text-align: center;">
	...
	// hapus data detail draft yang ada di modal
	...
	// hapus data per principle di detail kredit limit
	...
	$('#saveData').click(function(e) {
		...
		let kode = $("#kode").val();
		let add_id_kredit = $("#add_id_kredit").val();
		...
		Swal.fire({
			...
			if (result.value == true) {
				$.ajax({
					url: "<?= base_url('FAS/ManagementPelanggan/PengaturanKreditLimit/UpdateKreditLimit') ?>",
					type: 'POST',
					data: {
						pengajuan_kredit_id: add_id_kredit,
						pengajuan_kredit_kode: kode,
