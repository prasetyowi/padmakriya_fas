<script type="text/javascript">
    ...
    function SyncCustomer(idx, sistem_eksternal) {
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                //ajax save data
                $("#loadingview_" + idx).show();
                $("#btnsync_" + idx).hide();
                $(".btnsync").prop("disabled", true);

                $.ajax({
                    async: false,
                    url: "<?= base_url('FAS/ManagementPelanggan/SyncDataPelanggan/Insert_client_pt_principle_eksternal'); ?>",
                    type: "POST",
                    data: {
                        sistem_eksternal: sistem_eksternal
                    },
                    dataType: "JSON",
                    success: function(response) {
                        $.each(response, function(i, v) {
                            if (v == 1) {
                                let alert_tes = GetLanguageByKode('CAPTION-ALERT-GENERATEDSOFASSUCCESS');
                                message_custom("Success", "success", alert_tes);
                            } else if (v == 2) {
                                let alert_tes = GetLanguageByKode('CAPTION-ALERT-SOMESOFASCANNOTBEGENERATED');
