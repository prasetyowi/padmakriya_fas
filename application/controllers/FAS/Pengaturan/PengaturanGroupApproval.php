...
class PengaturanGroupApproval extends ParentController
{
	...
	public function __construct()
	{
		...
		$this->load->model('FAS/M_GroupApproval', 'M_GroupApproval');
		$this->load->model('M_Vrbl');

		$this->MenuKode = "299006000";
	}
	...
	public function PengaturanGroupApprovalMenu()
	{
		...
		$data['css_files'] = array(
			...
			Get_Assets_Url() . 'vendors/mjolnic-bootstrap-colorpicker/dist/css/bootstrap-colorpicker.min.css',
		...
		$data['js_files'] 	= array(
			...
			Get_Assets_Url() . 'vendors/mjolnic-bootstrap-colorpicker/dist/js/bootstrap-colorpicker.min.js',
		...
		$data['PT'] = $this->M_GroupApproval->getClientWMS($this->session->userdata('client_wms_id'));

		// Kebutuhan Authority Menu 
		$this->session->set_userdata('MenuLink', str_replace(base_url(), '', current_url()));

		$this->load->view('layouts/sidebar_header', $dataMenu);
		$this->load->view('FAS/Pengaturan/PengaturanGrupApproval/index', $data);
		$this->load->view('layouts/sidebar_footer', $dataMenu);
		$this->load->view('master/S_GlobalVariable', $data);
		$this->load->view('FAS/Pengaturan/PengaturanGrupApproval/script', $data);
	}
	...
	public function saveGroup()
	{
		...
		$this->M_GroupApproval->saveGroup($grup_id, $client_wms, $depo_id, $nama, $keterangan, $is_aktif, $tgl_create, $who_create);
	...
	public function getApprovalGrup()
	{
		...
		$data = $this->M_GroupApproval->getApprovalGrup($client_wms_id);
	...
	{
		$client_wms_id = $this->input->post('client_wms_id');
		$data = $this->M_GroupApproval->getKaryawan($client_wms_id);
