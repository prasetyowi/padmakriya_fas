...
class MainSession extends CI_Controller
{
	...
	public function MainSessionMenu()
	{
		...
		$karyawan_level_id	 = $this->input->post('karyawan_level_id');
		...
		$this->session->set_userdata('aplikasi_url', $aplikasi_url);
		$this->session->set_userdata('client_wms_id', $client_wms_id);
		$this->session->set_userdata('karyawan_divisi_id', $karyawan_divisi_id);
		$this->session->set_userdata('karyawan_level_id', $karyawan_level_id);
